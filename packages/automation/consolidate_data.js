      ...
      if (dpg.repositories.length > 0) {
        for (item in dpg.repositories) {
          if (dpg.repositories[item].name === "main") {
            repoIndex = item;
            var matchGithub = dpg.repositories[repoIndex].url.match(
              /https:\/\/github.com\/([^\/]*)\/([^\/]*)/
            );
            if (matchGithub) {
...
async function fetchGithubActivity(org, item) {
  let $,
    list = [];
  console.log(
    "Fetching https://github.com/" + org + " -> searching for " + item
  );
  let isOrg = true;
  let fetchLink =
  ...
  await retry(
    async (bail) => {
      ...
      if (data.status == 404) {
        // Assume handle is a personal account
        isOrg = false;
      } else {
      ...
      if (!isOrg) {
        fetchLink =
          "https://github.com/" + org + "/?tab=repositories&q=" + item;
      }
  ...
  if (isOrg) {
    // it is an organization, else it is a user
    list = $("div.repo-list")
      .find('a.d-inline-block:contains("' + item + '")')
      .filter(function () {
    ...
    list = $("#user-repositories-list")
      .find('a:contains("' + item + '")')
      .filter(function () {
        return $(this).text().trim() === item;
      })
      .parent()
